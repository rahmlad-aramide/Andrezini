<!DOCTYPE HTML>
<html lang="en">
    
<!-- Mirrored from Andrezini.kwst.net/site/portfolio-single.php by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 22 Nov 2022 08:54:42 GMT -->
<?php include("partials/head.php") ?>
    <body>
        <!--  main start  -->
        <div id="main">
            <!--  header main-header  -->
            <?php include("partials/header.php") ?>
            <!-- Header   end-->
            <!--  hero-wrap-social  -->
            <div class="social-container">
                <div class="hero-wrap-social">
                    <ul >
                        <li><a href="#" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
                        <li><a href="#" target="_blank"><i class="fab fa-instagram"></i></a></li>
                    </ul>
                </div>
            </div>
            <!--  hero-wrap-social end  -->			
            <!-- wrapper -->
            <div id="wrapper">
                <!-- content-holder -->
                <div class="content-holder">
                    <!-- fw-carousel-wrap -->
                    <div class="fw-carousel-wrap fsc-holder single_project_carousel">
                        <div class="fsc-holder_title">
                                <h2>Magodo<br>Residential</h2>
                            <div class="pr-det_btn">Project Details <i class="fas fa-long-arrow-right"></i></div>

                        </div>
                        <div class="fixed-bottom-panel">
                            <div class="ss-slider-btn ss-slider-prev"><i class="fal fa-long-arrow-left"></i></div>
                            <div class="ss-slider-btn ss-slider-next"><i class="fal fa-long-arrow-right"></i></div>
                            <div class="hs_init-container">
                                <div class="hs_init-wrap fl-wrap">
                                    <div class="hs_init"></div>
                                </div>
                            </div>
                        </div>
                        <!-- fw-carousel  -->
                        <div class="fw-carousel  thumb-contr fs-gallery-wrap fl-wrap full-height lightgallery" data-mousecontrol="true">
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="images/portfolio/MAGODO/MAGODO RESIDENTIAL Furniture 2.jpg" alt="Magodo Residential">
                                        <a href="images/portfolio/MAGODO/MAGODO RESIDENTIAL Furniture 2.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end-->  
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="images/portfolio/MAGODO/MAGODO RESIDENTIAL Interior & exterior design.jpg"   alt="Magodo Residential 2">
                                        <a href="images/portfolio/MAGODO/MAGODO RESIDENTIAL Interior & exterior design.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end-->                                      
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="images/portfolio/MAGODO/MAGODO RESIDENTIAL interior design 5.jpg"   alt="Magodo Residential 3">
                                        <a href="images/portfolio/MAGODO/MAGODO RESIDENTIAL interior design 5.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="images/portfolio/MAGODO/MAGODO RESIDENTIAL Interior design mgt 1.jpg"   alt=" Magodo Residential 4">
                                        <a href="images/portfolio/MAGODO/MAGODO RESIDENTIAL Interior design mgt 1.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="images/portfolio/MAGODO/MAGODO RESIDENTIAL Interior design mgt 4.jpg"   alt="Magodo Residential 5">
                                        <a href="images/portfolio/MAGODO/MAGODO RESIDENTIAL Interior design mgt 4.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="images/portfolio/MAGODO/MAGODO RESIDENTIALFurniture 3.jpg"   alt="Magodo Residential 6">
                                        <a href="images/portfolio/MAGODO/MAGODO RESIDENTIALFurniture 3.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODO_RESIDENTIAL_Interior_design_mgt_6_ozrn2t.jpg"   alt="Magodo Residential 7">
                                        <a href="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODO_RESIDENTIAL_Interior_design_mgt_6_ozrn2t.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODO_RESIDENTIAL_Interior_design_mgt_3_ro86wv.jpg"   alt="Magodo Residential 8">
                                        <a href="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODO_RESIDENTIAL_Interior_design_mgt_3_ro86wv.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODP_RESIDENTIALFurniture_1_s4sgtf.jpg"   alt="Magodo Residential 9">
                                        <a href="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODP_RESIDENTIALFurniture_1_s4sgtf.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide hov_zoom">
                                        <img  src="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODO_RESIDENTIAL_Interior_design_mgt_2_tttpau.jpg"   alt="Magodo Residential 10">
                                        <a href="https://res.cloudinary.com/dzhr3fddj/image/upload/q_50/v1672224653/Magodo%20residential/MAGODO_RESIDENTIAL_Interior_design_mgt_2_tttpau.jpg" class="box-media-zoom   popup-image"><i class="fal fa-search"></i></a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                    <!-- swiper-slide-->  
                                    <div class="swiper-slide ss_last" >
                                        <a class="next-project-wrap ajax fl-wrap" href="manhattan-club.php">
                                            <div class="ss_pag-img">
                                                <div class="bg"  data-bg="images/bg/1.jpg"></div>
                                            </div>
                                            <strong>Next Project</strong>
                                            <span>Manhattan Strip Club<i class="fal fa-long-arrow-right"></i></span>
                                        </a>
                                    </div>
                                    <!-- swiper-slide end--> 
                                </div>
                            </div>
                        </div>
                        <!-- fw-carousel end -->
                    </div>
                    <!--thumbnail-container-->	
                    <div class="thumbnail-container">
                        <div class="thumbnail-wrap fl-wrap">
                        </div>
                    </div>
                    <!--thumbnail-container end-->	
                    <!--project-details -->
                    <div class="project-details_inner">
                        <div class="project-details_overlay cpd"></div>
                        <div class="project-details_wrap">
                            <div class="pd_modal-close cpd  color-bg"><i class="fal fa-times"></i></div>
                            <div class="project-details fl-wrap">
                                <div class="project-details_container fl-wrap">
                                    <div class="section-title fl-wrap">
                                        <h2>Project description</h2>
                                        <p>Magodo Residential</p>
                                        <div class="sec-title_dec"></div>
                                    </div>
                                    <div class="modal-content fl-wrap">
                                        <p>TURNKEY PROJECT: We fully
                                            designed the project. <br> Our services
                                            includes: <br>
                                            1) Design consultation (3d visuals). <br>
                                            2) Furniture consultancy/
                                            Fabrication. <br>
                                            3) Interior & Exterior project. <br>
                                            4) Architecture & Construction
                                            services. <br>
                                            Interior electrical & Lighting.</p>
                                    </div>
                                    <div class="pr-list dark-bg fl-wrap">
                                        <ul>
                                            <li><span>Status :</span> Completed </li>
                                            <li><span>Location : </span> Magodo Lagos </li>
                                        </ul>
                                        <span class="pr-list_dec"></span>
                                    </div>
                                    <a href="#" class="btn    color-bg  fl-btn" target="_blank"><span>View Project</span></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!--project-details end-->
                    <!-- side-box --> 
                    <div class="side-box">
                        <a href="portfolio.php" class="ajax aside-link"><i class="fas fa-th"></i><span>Projects</span></a>
                        <div class="single-page_scrl">
                            <div class="mousey">
                                <div class="scroller"></div>
                                <i class="fas fa-angle-down"></i>
                            </div>
                        </div>
                    </div>
                    <!-- side-box   end --> 
                    <!-- share-wrapper-->
                    <div class="share-wrapper  isShare">
                        <div class="close-share-btn"><i class="fal fa-long-arrow-up"></i></div>
                        <div class="share-container fl-wrap  isShare"></div>
                    </div>
                    <!-- share-wrapper  end -->				
                </div>
                <!-- content-holder end -->
            </div>
            <!-- wrapper end -->
            <footer class="main-footer">
                <!--  copy-right  -->
                <div class="copy-right"> <span>&#169; </span>   Andrezini 2022  /  All rights reserved.  </div>
                <!--  copy-right end  -->			
                <!-- header-contacts--> 
                <div class="footer-contacts">
                    <ul>
                        <li><span>Call </span> <a href="tel:+234 816 661 1795">+234 816 661 1795</a></li>
                        <li><span>Email </span> <a href="mailto:info@andrezini.com">info@andrezini.com</a></li>
                    </ul>
                </div>
                <!-- header-contacts end--> 			
            </footer>
            <div class="right-dec"></div>
            <!-- progress-bar  -->
            <div class="progress-bar-wrap hide_pw">
                <div class="progress-bar color-bg"></div>
            </div>
            <!-- progress-bar end -->			
        </div>
        <!-- Main end -->
        <!--=============== scripts  ===============-->   
        <script  src="js/jquery.min.js"></script>
        <script  src="js/plugins.js"></script>
        <script  src="js/scripts.js"></script>
    </body>

<!-- Mirrored from Andrezini.kwst.net/site/portfolio-single.php by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 22 Nov 2022 08:54:42 GMT -->
</html>